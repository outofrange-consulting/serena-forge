...
scan() { # $1=fichier  $2=lignes ajoutees separees par espace, ou "ALL"
  # Dans un projet *ApiModels, /// est libre sur tous les membres : les descriptions
  # OpenAPI (parametres + schemas) en sont extraites, le lint redocly les exige.
  local am=0
  local p
  p="$(realpath "$1" 2>/dev/null || echo "$1")"
  # Migrations : code genere par EF (en-tetes <auto-generated /> / <inheritdoc />), jamais ecrit a la main.
  case "$p" in */Migrations/*) return 0 ;; esac
  case "$p" in *ApiModels/*|*ApiModel/*) am=1 ;; esac
  awk -v F="$1" -v ADD="$2" -v AM="$am" '
    ...
    d {
      if (AM != "1" &&
          code !~ /(^|[^A-Za-z])(record|enum)([^A-Za-z]|$)/ &&

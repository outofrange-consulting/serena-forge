...
globally: direct `Edit`/`Write`/`MultiEdit` on `.cs` files is denied and redirects you to Serena's symbolic/file edits. So a repo you actually want to change in C# must be onboarded first, or native `.cs` edits remain blocked.
...
Because native `.cs` writes are denied everywhere, an un-onboarded repo cannot be
edited safely until this skill runs. When C# work is requested in such a repo (no
...
Because native `.cs` writes are denied globally, a repo where Serena's LSP won't come
